...
<body data-offset="200" data-spy="scroll" data-target=".ow-navigation">
	...
	<main> <!-- Page Banner -->
	<div
		class="page-banner gallerysingle-banner container-fluid no-padding">
			...
			<div class="sub-main-w3">
				<form id="appointment" onsubmit="return false">
					...
					<div class="form-style-agile">
						<input placeholder="你的名字" name="apppname" type="text" required="">
					</div>
					<div class="form-style-agile">
						<input placeholder="电话号码" name="appphone" type="text" required="">
					</div>
					<div class="form-style-agile">
						<input placeholder="身份证号" name="appIdCard" type="text" required="">
					</div>
					<div class="form-style-agile">
						<select class="category" id="dept" required="" name="deptid">
							<option value="">《---请选择科室---》</option>
						</select>
					</div>
					<div class="form-style-agile">
						<select class="category" required="" id="doctor" name="docid">
							<option value="">《---请选择医生（非必需）---》</option>
					...
					<div class="form-style-agile">
						<select class="category" required="">
							<option value="">2019-02-25</option>
							<option value="">2019-02-26</option>
							<option value="">0219-02-37</option>
						</select>
					</div>
					<div class="form-style-agile">
						<select class="category" required="">
							<option value="">09:00-10:00（剩余10）</option>
							<option value="">10:00-11:00（剩余10）</option>
							<option value="">11:00-12:00（剩余10）</option>
							<option value="">14:00-15:00（剩余10）</option>
							<option value="">15:00-16:00（剩余10）</option>
							<option value="">16:00-17:00（剩余10）</option>
							<option value="">17:00-18:00（剩余10）</option>
						</select>
					</div>
					<input type="submit" value="确认预约" onclick="appointmentAjax()">
				</form>
			</div>
		</div>
		<!-- //content -->
	...
	<script type="text/javascript">
		...
		//预约Ajax
		function appointmentAjax() {
			alert($("#appointment").serialize())
			$.post('/pet_hospital/appointment', $("#appointment").serialize(),
					function(data, status) {
						if (status == 'succeed' || data == 'succeed') {
							alert('预约成功，如不能及时到达，请提前一个小时打电话取消预约')
						} else {
							alert('预约失败，请重新预约')
						}
					});
		...
		//查询科室
		function dept() {
			$.get('/pet_hospital/selectDept', function(data, status) {
				json = JSON.parse(data)
				$.each(json,function(idx,obj){
					$("#dept").append('<option value="'+obj.deptid+'">'+obj.deptname+'</option>')
				});
			});
		}
		//查询医生
		  $("#dept").change(function(){
			  ...
			  $.get('/pet_hospital/doctorFindByDeptid/'+$(this).val(), function(data, status) {
					json = JSON.parse(data)
					$.each(json,function(idx,obj){
						$("#doctor").append('<option value="'+obj.docid+'">'+obj.docname+'</option>')
					});
				});
			  });
	</script>
